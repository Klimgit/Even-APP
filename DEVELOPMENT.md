...
just test-integration  # docker stack + smoke-api (как в GitHub Actions)
just smoke-api         # только HTTP smoke (сервисы уже запущены)
```

В CI ([`.github/workflows/ci.yml`](.github/workflows/ci.yml)): **unit** → **build** → **integration**. Deploy и Deploy Preview ждут успешного CI.
...
### Рекомендуемый порядок (локально и в CI)
