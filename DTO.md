...
Хранится в **even_media**: `scope=platform` — общая база платформы (только admin пишет); `scope=teacher` — личная база учителя (изоляция по `owner_id`). Lexicon и content ссылаются на `media_asset_id` по UUID без FK.
